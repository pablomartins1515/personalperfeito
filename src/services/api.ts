...
type APIInstanceProps = AxiosInstance & {
	registerInterceptTokenMananger: ({}: RegisterInterceptTokenManagerProps) => () => void;
}

const api = axios.create({
	baseURL: "http://18.231.160.10:3333",
}) as APIInstanceProps;
...
api.registerInterceptTokenMananger = ({signOut, refreshedTokenUpdated}) => {
	const interceptTokenMananger = api.interceptors.response.use(response => response, async requestError => {
			...
			return () => {
				api.interceptors.response.eject(interceptTokenMananger);
			};
		};
